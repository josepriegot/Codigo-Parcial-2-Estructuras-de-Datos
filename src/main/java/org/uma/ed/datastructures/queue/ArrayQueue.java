...
public class ArrayQueue<T> extends AbstractQueue<T> implements Queue<T> {
   ...
   * - Elements in queue are stored in array one after the other starting at index `first` and ending at index `last`.
   *   If we reach the end of the array, we continue at the beginning.
   */

  /**
   * Creates an empty ArrayQueue. Initial capacity is {@code initialCapacity} elements. Capacity is automatically
  ...
  /**
   * Creates an empty ArrayQueue with default initial capacity. Capacity is automatically increased when needed using a
   * doubling strategy.
   * <p> Time complexity: O(1)
   */
  public ArrayQueue() {
  ...
  public static <T> ArrayQueue<T> of(T... elements) {
    ArrayQueue<T> queue = ArrayQueue.withCapacity(elements.length);
  ...
  /**
   * Returns the index in array of next element after given one. If given index is last one, returns 0.
  ...
  /**
   * Ensures that the capacity of the ArrayQueue is sufficient to hold a new element.
